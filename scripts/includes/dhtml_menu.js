...
Drupal.behaviors.dhtmlMenu = function() {
  ...
  if (Drupal.settings.dhtmlMenu.colors) {  
	for (var i in Drupal.settings.dhtmlMenu.colors) {
		if (Drupal.settings.dhtmlMenu.colors[i].isFirst) {			
			$('li' + i).find('a:first').css('color', Drupal.settings.dhtmlMenu.colors[i].color); 
		} else {
			$('li' + i).find('a').css('color', Drupal.settings.dhtmlMenu.colors[i].color); 
		}
	}
	
	//$('li.dhtml-menu').find('a').css({'color':'#'}); 
	//$('li.dhtml-menu.active-trail').find('a:first').css('color', '#000000');
  }
}
...
Drupal.dhtmlMenu.toggleMenu = function(li) {
  ...
  if (effects.collapseOpen) {
	  var p=$(li).parent();
		
	  if (effects.slide) {	  
		$(p).find('li.dhtml-menu:not(.active-trail).expanded').removeClass('expanded').addClass('collapsed').find('ul:first').animate({height: 'hide', opacity: 'hide'}, '1000').css('display', 'none');
	  } else {
		$(p).find('li.dhtml-menu:not(.active-trail).expanded').removeClass('expanded').addClass('collapsed').find('ul:first').css('display', 'none').removeClass('expanded').addClass('collapsed');
	  }
  }
  ...
  //$('li.collapsed').find('a:first').css({'color':'#000000'});
  //$('li.expanded').find('a:first').css({'color':'#FF0000'});
}
